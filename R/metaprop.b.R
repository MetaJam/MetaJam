metaPropClass <- R6::R6Class(
  "metaPropClass",
  inherit = metaPropBase,
  ...
  active = list(
    model = function() {
      if (isFALSE(private$.model)) {
        private$.model <- NULL
        private$.model <- computePropModel(self)
      }
      private$.model
    },
    ...
    subgroupModels = function() {
      if (isFALSE(private$.subgroupModels)) {
        ...
        private$.subgroupModels <- computePropSubgroupModels(self)
      }
  ...
  private = list(
      ...
      runSafe(
        {
          ...
          updateForestSize(
            ...
            renderCall = function() renderPropForest(self, sortKey = sortKey)
          )
    ...
    .forestPlot = function(image, ...) {
      renderPropForest(self, sortKey = image$state$sortKey)
    },
