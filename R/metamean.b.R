metaMeanClass <- R6::R6Class(
  "metaMeanClass",
  inherit = metaMeanBase,
  ...
  active = list(
    model = function() {
      if (isFALSE(private$.model)) {
        private$.model <- NULL
        private$.model <- computeMeanModel(self)
      }
      private$.model
    },
    ...
    subgroupModels = function() {
      if (isFALSE(private$.subgroupModels)) {
        ...
        private$.subgroupModels <- computeMeanSubgroupModels(self)
      }
  ...
  private = list(
      ...
      runSafe(
        {
          ...
          updateForestSize(
            ...
            renderCall = function() renderMeanForest(self, sortKey = sortKey)
          )
    ...
    .forestPlot = function(image, ...) {
      renderMeanForest(self, sortKey = image$state$sortKey)
    },
