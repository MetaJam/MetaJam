metaRateClass <- R6::R6Class(
  "metaRateClass",
  inherit = metaRateBase,
  ...
  active = list(
    model = function() {
      if (isFALSE(private$.model)) {
        private$.model <- NULL
        private$.model <- computeRateModel(self)
      }
      private$.model
    },
    ...
    subgroupModels = function() {
      if (isFALSE(private$.subgroupModels)) {
        ...
        private$.subgroupModels <- computeRateSubgroupModels(self)
      }
  ...
  private = list(
      ...
      runSafe(
        {
          ...
          updateForestSize(
            ...
            renderCall = function() renderRateForest(self, sortKey = sortKey)
          )
    ...
    .forestPlot = function(image, ...) {
      renderRateForest(self, sortKey = image$state$sortKey)
    },
