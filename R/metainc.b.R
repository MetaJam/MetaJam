metaIncClass <- R6::R6Class(
  ...
  active = list(
    ...
    subgroupModels = function() {
      if (isFALSE(private$.subgroupModels)) {
        ...
        private$.subgroupModels <- computeIncSubgroupModels(self)
      }
  ...
  private = list(
      ...
      runSafe(
        {
          ...
          updateForestSize(
            ...
            renderCall = function() renderIncForest(self, sortKey = sortKey)
          )
    ...
    .forestPlot = function(image, ...) {
      renderIncForest(self, sortKey = image$state$sortKey)
    },
